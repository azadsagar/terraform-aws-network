locals {
  ...
  vpc_flow_logs_bucket = length(var.vpc_flow_logs_default_bucket) == "" ? "${local.tag_prefix}-${data.aws_caller_identity.this.account_id}-vpc-flow-logs" : var.vpc_flow_logs_default_bucket
}